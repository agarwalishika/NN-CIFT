...
class UsingInfluence:
    def __init__(self, influence: LearningInfluence, influence_network_subset_perc, subset_selection_perc, use_delift) -> None:
        ...
        self.language_model_name = "meta-llama/Llama-3.1-8B"
    ...
    def create_subset(self, subset_selec_perc, influence_network_perc, use_delift=False):
        if self.influence.type == LESS_CONSTANT:
            return self.create_less_subset(subset_selec_perc, influence_network_perc, use_delift)
        start = time.time()
        ...
        if use_delift:
            ...
            existing_embeddings_d2 = self.embedding_model.encode(self.influence.existing_data[dl:], normalize_embeddings=True)
            new_embeddings = self.embedding_model.encode(self.influence.new_data[pl:], normalize_embeddings=True)

            data_sijs, private_sijs = [], []
            for i in range(len(existing_embeddings_d1)):
                ...
                for j in range(len(existing_embeddings_d2)):
                    ...
                    data_sijs[i].append(data_influence(torch.from_numpy(input)).detach().cpu())
        ...
        if self.influence.type == ICL_CONSTANT or self.influence.type == SE_CONSTANT:
            data_sijs = np.array(data_sijs).squeeze()
            private_sijs = np.array(private_sijs).squeeze()
            ...
            subset = fl.maximize(budget=int(subset_selec_perc * n), optimizer='LazyGreedy', stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False)
            subset = np.array(subset)[:, 0].astype(np.int32)
        elif self.influence.type == RANDOM_CONSTANT:
            subset = np.random.randint(0, data_sijs.shape[0], int(subset_selec_perc * n))
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--order', type=str, default="")
    argparser.add_argument('--type', type=str, default=ICL_CONSTANT)
    ...
    #     subset_selection_subset = [0, 0.1, 0.2, 0.3, 0.9999999]
    # elif args.type == "extreme":
    #     subset_selection_subset = [0.0, 0.9999999]
    #     args.type = ICL_CONSTANT
    # else:
